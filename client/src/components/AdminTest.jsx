...
const AdminTest = () => {
  ...
  const handleUpdateTest = async () => {
    try {
      const response = await axios.put(`http://localhost:3002/api/updateTest/${currentTest._id}`, currentTest);
  ...
  return (
    <div className="admin-test-table-container">
      <div className="admin-test-search-container">
        ...
        />
        <button className="admin-test-add-button" onClick={() => setIsAddModalOpen(true)}>Add Test</button>
      </div>
        ...
        <tbody>
          {AdminTest.map((Test) => (
            <tr className="admin-test-tr" key={Test._id}>
              ...
              <td className="admin-test-td description">{Test.description}</td>
              <td className="admin-test-td">
                <button className="admin-test-view" onClick={() => handleViewTest(Test._id)}>
                  <i className="fas fa-eye"></i>
                </button>
                <button className="admin-test-edit" onClick={() =>  handleEditClick(Test)}>
                  <i className="fas fa-edit"></i>
                </button>
                <button className="admin-test-delete" onClick={() => handleDeleteTest(Test._id)}>
      ...
      {isAddModalOpen && (
        <div className="modal">
          <div className="modal-content">
            ...
            <form onSubmit={(e) => {
              e.preventDefault();
              handleAddTest({ name: e.target.name.value, description: e.target.description.value });
            }}>
              <input type="text" name="name" placeholder="Test Name" required />
              <textarea name="description" placeholder="Test Description" required />
  ...
  <div className="modal">
    <div className="modal-content">
      ...
      <form>
        ...
        <button type="submit" onClick={handleUpdateTest}>Save Changes</button>
